...
메시지에서 `@그룹명`을 입력하면 플러그인이 그룹 멘션을 검사합니다:
...
2. 메시지는 그대로 저장: `팀 미팅 15분 후 시작합니다! @dev`
3. Mattermost Team Edition에서는 커스텀 그룹명만으로 네이티브 멘션 알림이 보장되지 않을 수 있음

**네이티브 알림이 꼭 필요한 경우 (`text-expand`):**
1. 입력한 메시지: `팀 미팅 15분 후 시작합니다! @dev`
2. 자동 확장되어 저장: `팀 미팅 15분 후 시작합니다! @김철수 @이영희 @박민수`
3. Mattermost 네이티브 시스템이 개별 사용자 멘션 알림 전송

**참고:**
- `text-expand` 모드에서는 채널에 있는 그룹 멤버에게만 알림 전송
- 메시지 작성자는 자동으로 제외
- `notify-only` 모드는 그룹명이 본문에 그대로 남는 것을 우선합니다
- `text-expand` 모드는 안정적인 네이티브 알림을 우선합니다
...
플러그인은 **MessageWillBePosted** 훅을 사용하여 메시지가 저장되기 **전에** 그룹 멘션을 검사합니다. 동작 방식은 **Mention Expansion Mode** 설정에 따라 달라집니다.

- `notify-only` (기본값): 메시지 본문에 입력한 `@그룹명`을 그대로 유지합니다. Mattermost Team Edition에서는 본문에 실제 사용자 `@mention`이 없으면 네이티브 멘션 알림이 보장되지 않을 수 있습니다.
- `text-expand`: 메시지가 저장되기 전에 `@그룹명`을 개별 `@사용자명`으로 확장합니다. 이 방식은 Mattermost 네이티브 멘션 알림을 가장 안정적으로 발생시키지만, 채팅 본문에는 그룹명 대신 사용자 멘션 목록이 표시됩니다.

### 기술적 동작 방식 (`text-expand`)
...
- ✅ 채팅 본문에 `@그룹명`이 그대로 남음
- ✅ 멤버 목록이 메시지에 노출되지 않음
- ⚠️ Mattermost Team Edition에서 네이티브 멘션 알림이 보장되지 않을 수 있음
...
| **Mention Expansion Mode** | notify-only | `notify-only`: 그룹명 유지, 네이티브 알림 미보장 / `text-expand`: 사용자 멘션으로 확장, 네이티브 알림 안정적 |
...
7. **멘션 모드 선택**: 본문 유지와 네이티브 알림 중 우선순위를 정하세요
   - `notify-only`: `@그룹명` 유지, 네이티브 알림 미보장
   - `text-expand`: `@사용자명`으로 확장, 네이티브 알림 안정적
...
- 플러그인 설정의 **Mention Expansion Mode** 확인
- Mattermost 네이티브 멘션 알림이 필요하면 `text-expand` 사용
...
- 플러그인 설정이 `text-expand` 모드일 때의 정상 동작입니다
- Mattermost 네이티브 멘션 알림을 안정적으로 보내기 위해 메시지 저장 전에 확장합니다
- 그룹명을 그대로 유지하려면 **Mention Expansion Mode**를 `notify-only`로 변경하세요
- 단, `notify-only`에서는 Mattermost Team Edition의 네이티브 멘션 알림이 보장되지 않을 수 있습니다
...
**Q: DM으로 알림이 오나요?**
- 기본 동작은 DM 전송이 아닙니다
- `text-expand` 모드에서는 일반 @멘션과 동일한 방식으로 알림이 전송됩니다 (모바일 푸시, 데스크톱 알림)
